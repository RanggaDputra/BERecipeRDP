...
const getRecipeAll = async () => {
    ...
    return new Promise((resolve,reject)=>
        Pool.query(`SELECT recipe.id, recipe.title, recipe.ingredients, recipe.photo, category.name AS category, users.username AS author,users.photo AS author_photo FROM recipe JOIN category ON recipe.category_id = category.id JOIN users ON recipe.users_id = users.id`,(err,result)=>{
            if(!err){
                resolve(result)
            } else{
                reject(err)
...
const getRecipeByUsers = async (id) => {
    ...
    return new Promise((resolve, reject) =>
        Pool.query(
            `SELECT recipe.id, recipe.title, recipe.ingredients, recipe.photo, category.name AS category, users.username AS author,users.photo AS author_photo FROM recipe JOIN category ON recipe.category_id = category.id JOIN users ON recipe.users_id = users.id WHERE users_id=${id} ORDER BY recipe.id DESC`,
            (err, result) => {
                if (!err) {
                    resolve(result);
                } else {
...
module.exports =  {getRecipe,getRecipeById,deleteById,postRecipe,putRecipe,getRecipeAll,getRecipeCount,getRecipeByUsers}