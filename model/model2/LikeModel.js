...
const getMyLikeMenu = async (id) => {
    return new Promise((resolve, reject)=>{
        ...
        pool.query(`SELECT recipe.id AS recipe_id, liked.id AS liked_id, recipe.title, recipe.users_id, recipe.photo AS photo_menu, recipe.ingredients, category.name AS category, liked.user_id AS liked_by, users.username, users.photo AS photo_user, liked.created_at FROM liked JOIN recipe ON liked.recipe_id = recipe.id JOIN users ON recipe.users_id = users.id JOIN category ON recipe.category_id = category.id WHERE liked.user_id = ${id}`, (err, results) => {
            if (!err){
                resolve(results)
            } else {
                reject(err)
