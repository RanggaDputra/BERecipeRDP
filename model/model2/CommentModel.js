...
const getComment = async (id) => {
    ...
    return new Promise((resolve, reject)=>{
        ...
        pool.query(`SELECT comment.id, comment.user_id, users.username, users.photo, comment.text FROM comment JOIN users ON comment.user_id = users.id JOIN recipe ON comment.recipe_id = recipe.id WHERE comment.recipe_id=${id} ORDER BY comment.created_at ASC`, (err, results) => {
            if (!err){
                resolve(results)
            } else {
                reject(err)
...
const getCommentEdit = async (id) => {
    ...
    return new Promise((resolve, reject)=>{
        ...
        pool.query(`SELECT comment.id, comment.user_id, users.username, users.photo, comment.text FROM comment JOIN users ON comment.user_id = users.id JOIN recipe ON comment.recipe_id = recipe.id WHERE comment.id=${id}`, (err, results) => {
            if (!err){
                resolve(results)
            } else {
                reject(err)
