...
const getMyBookmarkMenu = async (id) => {
    return new Promise((resolve, reject)=>{
        ...
        pool.query(`SELECT recipe.id AS recipe_id, bookmarked.id AS bookmarked_id, recipe.title, recipe.users_id, recipe.photo AS photo_menu, recipe.ingredients, category.name AS category, bookmarked.user_id AS bookmarked_by, users.username, users.photo AS photo_user, bookmarked.created_at FROM bookmarked JOIN recipe ON bookmarked.recipe_id = recipe.id JOIN users ON recipe.users_id = users.id JOIN category ON recipe.category_id = category.id WHERE bookmarked.user_id = ${id}`, (err, results) => {
            if (!err){
                resolve(results)
            } else {
                reject(err)
